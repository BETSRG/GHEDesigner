...
def main():
    ...
    max_HP_EFT, min_HP_EFT = HourlyGLHE.simulate()
    ...
    lntts = [np.log(time_values[i] * 3600 / HourlyGLHE.radial_numerical.t_s)
             for i in range(len(time_values))]

    B_over_H = B / HourlyGLHE.bhe.b.H
    g = HourlyGLHE.grab_g_function(B_over_H)

    ax.plot(g.x, g.y)
    ax.plot(lntts, HourlyGLHE.delta_Tb, '--')
