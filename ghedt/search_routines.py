...
class RowWiseModifiedBisectionSearch:
    ...
    def search(self,eT = 1e-10):
        ...
        # in the user's best interest to return a field smaller than the smallest one. This is done by removing rows
        #from the field.

        elif T_lower < 0.0 and T_upper < 0.0:
            #The most efficient field is most likely the one utilizing the maximum target depth. As such, rows will be
            #removed until the height used is as close to the maximum as possible.
        ...
        #calculation.
            ...
            msg = 'There seems to be an issue calculating excess temperatures. Check that you have the correct' \
                  'package version. If the issue is recurring, please contact the current package management for ' \
                  'assistance.'
            raise ValueError(msg)
        self.excess = selected_temp_excess
        return selected_coordinates,selected_specifier
    def RemoveRow(self,field):
        #The crux of this function is predicated on the fact that the points generated by RowWise are stored in rows.
        #Because of this fact, we can figure out where row partitions are by looking for a change in the sign of
        #dx and dy between point pairs.

        #There could be some really difficult edge cases to work around (or maybe the functionality overall
        # could be insufficient).
        #One option to fix this could be to
        #restructure RowWise to keep track of rows (a 2d array). That would make this function simpler (and more robust),
        #but might require large changes to RowWise.
        ...
        for i in range(numberOfPoints-1):
            ...
            if not(last_dx == None) and not(last_dy == None):
                if last_dx != dx or last_dy != dy:
                    #Trying to catch the edge case where the first "row" contains only one point.
                    if i == 1:
                        newField = field[1:len(field)]
                        keepLooping = False
                    #No other edge cases currently accounted for (you can add them here).
                    else:
                        #Because we are looking at the current point and the one ahead, the current point is part of
                        #the row that should be removed. A slice from the next point to the end of the field array
                        #is selected.
                        newField = field[i+1:len(field)]
                        keepLooping = False

            #Making sure that the loop acutally stops when a new field is selected.
            last_dx = dx
            last_dy = dy
            if not keepLooping:
                break
        #Trying to catch the edge case where there is only one row remaining.
        if keepLooping:
            newField = field
        return newField
