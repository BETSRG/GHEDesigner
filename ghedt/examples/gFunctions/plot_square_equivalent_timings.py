...
def main():
    ...
    calculations = ['8_Unequal_Segments_Equivalent_MIFT',
                    '12_Equal_Segments_Equivalent_MIFT']

    submission_folder = r'Submission_Files/96m_Depth/'
    ...
    for j in range(len(calculations)):
        ...
        timing_data[calculation] = {'nbh': [], 'time':  []}
        ...
        for i in range(len(submission_files)):
            ...
            gfunc = ghedt.gfunction.calculate_g_function(
                ...
                segments=segments, solver=solver, boundary=boundary, disp=disp)
            toc = clock()
            computation_time = toc - tic

            timing_data[calculation]['nbh'].append(len(coordinates))
            timing_data[calculation]['time'].append(computation_time)
    ...
    for calculation in calculations:
        data = timing_data[calculation]
        ax.scatter(data['nbh'], data['time'],
                   label=calculation.replace('_', ' ').replace('MIFT', 'UIFT'),
                   marker=markers[i])
        i += 1
    ...
    fig.legend(bbox_to_anchor=(0.6, 0.97))
    ...
    fig.savefig('square_equivalent_timings.png')
