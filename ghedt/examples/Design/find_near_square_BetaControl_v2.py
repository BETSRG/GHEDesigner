...
import csv
import ghedt.gfunction as gfunction
import ghedt.ground_heat_exchangers as ground_heat_exchangers
...
def main():
    ...
    for mnVal in mnVals:
        ...
        #print("Mass Flow Rate: ",mflow)
        coords = []
        for mVal in range(m):
            for nVal in range(n):
                coords.append([mVal*B,nVal*B])
        #print("Coordinates: ",coords)
        #print("Coord Number: " + str(m*n) + ", Actual Coord Number: " + str(len(coords)))
        gfunction_obj = gfunction.compute_live_g_function(B,[24,48,96,192,384],[.075,.075,.075,.08,.0875],[2,2,2,2,2],
                                                          mflow,single_u_tube,logTime,coords,fluid,pipe_single,grout,soil)

        ghe = ground_heat_exchangers.GHE(V_flow, B, single_u_tube, fluid, borehole, pipe_single,
                                         grout, soil, gfunction_obj, sim_params, hourly_extraction_ground_loads)
        ghe.size()
