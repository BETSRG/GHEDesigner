...
def main():
    ...
    lMNumber = 1
    tolerance = 1e-10
    lmStep = (lMStop-lMStart)/lMNumber
    lMVals = []
    cLM = lMStart
    while cLM <= lMStop + tolerance and len(lMVals)<lMNumber:
        lMVals.append(cLM)
        cLM+=lmStep
    outA = []
    outA.append(["Load Multiplier","Field","Field Depth","Excess Temperature","Field Length","Calculation Time"])
    print(lMVals)
    for lMVal in lMVals:
        # Borehole dimensions
        # -------------------
        ...
        k_s = 2.8  # Ground thermal conductivity (W/m.K)
        k_g = 1.0  # Grout thermal conductivity (W/m.K)

        # Volumetric heat capacities
        # --------------------------
        rhoCp_p = 1542. * 1000.  # Pipe volumetric heat capacity (J/K.m3)
        rhoCp_s = 2600 * 1000.  # Soil volumetric heat capacity (J/K.m3)
        ...
        pipe_coaxial = \
            plat.media.Pipe(pos_coaxial, r_inner, r_outer, 0, epsilon, k_p_coax,
                            rhoCp_p)
        # Soil
        ugt = 17.3  # Undisturbed ground temperature (degrees Celsius)
        ...
        hourly_extraction: dict = \
            pd.read_csv('../SingleBuildingLoads.csv').to_dict('list')
        # Take only the first column in the dictionary
        hourly_extraction_ground_loads: list = \
            hourly_extraction[list(hourly_extraction.keys())[0]]
        hourly_extraction_ground_loads = lMVal*np.array(hourly_extraction_ground_loads)
        hourly_extraction_ground_loads = list(hourly_extraction_ground_loads)

        propertyBoundary: dict = \
                pd.read_csv('../Propbound.csv').to_dict('list')
        propertyBoundary: list = \
                [[propertyBoundary['x'][i],propertyBoundary['y'][i]] for i in range(len(propertyBoundary['x']))]
        nogoZone: dict = \
            pd.read_csv('../OSU_Building_NogoZone.csv').to_dict('list')
        nogoZone: list = \
            [[nogoZone['x'][i],nogoZone['y'][i]] for i in range(len(nogoZone['x']))]
        ...
        design_single_u_tube = dt.design.Design(
            ...
            flow=flow, routine='bi-rectangle_constrained',property_boundary=propertyBoundary,buildingDescription=nogoZone)
        ...
        outA.append([lMVal,bisection_search.ghe.fieldSpecifier,bisection_search.ghe.bhe.b.H,bisection_search.ghe.excessTemperature,bisection_search.ghe.nbh,toc-tic])
        ...
        print('Total Drilling: {0:.1f} meters\n'.
              format(bisection_search.ghe.bhe.b.H * nbh))
        '''
    with open("LoadingSweep.csv","w",newline="") as outputFile:
