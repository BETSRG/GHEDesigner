...
def OutputDesignDetails(design,time,projectName,notes,author,modelName,allocatedWidth = 100,roundingAmount=10,
    ...
    oS += createDRow(allocatedWidth, "Borehole Spacing, m:", round(ghe.B_spacing,roundingAmount), stringFormat, floatFormat)
    oS += createDRow(allocatedWidth, "Total Drilling, m:", round(bH.H*len(b), roundingAmount), stringFormat,floatFormat)
    ...
    oS += createDRow(allocatedWidth, "Field Specifier:", ghe.fieldSpecifier, stringFormat, stringFormat,bTabs=indentedAmount)
    ...
    oS += "Soil Properties: " + "\n"
    oS += createDRow(allocatedWidth, "Thermal Conductivity, W/(m*K):", round(bhe.soil.k,roundingAmount), stringFormat, floatFormat,bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Volumetric Heat Capacity, kJ/(K*m^3):", round(bhe.soil.rhoCp/1000,roundingAmount), stringFormat, floatFormat,bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Undisturbed Ground Temperature, C:", round(bhe.soil.ugt,roundingAmount), stringFormat, floatFormat,bTabs=indentedAmount)
    ...
    oS += createDRow(allocatedWidth, "Maximum Allowable HPEFT, C: ", ghe.sim_params.max_EFT_allowable,stringFormat,floatFormat)
    oS += createDRow(allocatedWidth, "Minimum Allowable HPEFT, C: ", ghe.sim_params.min_EFT_allowable,stringFormat,floatFormat)
        ...
        if currentMonth == lastMonth:
            ...
            if len(monthTbVals) > 0:
                ...
                if len(outArray)==0:
                    ...
                    previousTemp = bhe.soil.ugt
                outArray.append([currentMonth,previousTemp + monthTbVals[-1],np.max(monthEFTVals),np.min(monthEFTVals)])
    ...
    oS += createDRow(allocatedWidth, "Max HPEFT Time, Months:", round(maxEFTTime, roundingAmount), stringFormat, floatFormat)
