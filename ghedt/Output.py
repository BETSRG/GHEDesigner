...
def createTable(title, colTitles, rows, allowcatedWidth, colFormats, fillerSymbol=" ", centering=">"):
...
def createDRow(rowAllocation, Entry1, Entry2, dType1, dType2, bTabs=0, aTabs=0):
    ...
    for t in range(bTabs):
        rS += "\t"
    rS += "{:<{lO}{f1}}{:>{rO}{f2}}".format(Entry1, Entry2, lO=leftOffset, rO=rightOffset, f1=dType1, f2=dType2)
...
def OutputDesignDetails(design, time, projectName, notes, author, modelName, allocatedWidth=100, roundingAmount=10,
    ...
    oS += createDRow(allocatedWidth, "Project Name:", projectName, stringFormat, stringFormat)
    oS += blankLine
    oS += "Notes:\n\n" + notes + "\n"
    oS += blankLine
    oS += createDRow(allocatedWidth, "File/Model Name:", modelName, stringFormat, stringFormat)
    now = datetime.now()
    timeString = now.strftime("%m/%d/%Y %H:%M:%S %p")
    oS += createDRow(allocatedWidth, "Simulated On:", timeString, stringFormat, stringFormat)
    oS += createDRow(allocatedWidth, "Simulated By:", author, stringFormat, stringFormat)
    oS += createDRow(allocatedWidth, "Calculation Time, s:", round(time, roundingAmount), stringFormat, floatFormat)
    oS += emptyLine
    ...
    oS += createTable("Field Search Log", designHeader, designValues, allocatedWidth, designFormats,
    ...
    oS += createTable("GFunction LTS Values", [gfunctionColTitles], gfunctionData, allocatedWidth,
    ...
    oS += createDRow(allocatedWidth, "Active Borehole Length, m:", bH.H, stringFormat, intFormat)
    oS += createDRow(allocatedWidth, "Borehole Radius, m:", round(bH.r_b, roundingAmount), stringFormat, floatFormat)
    oS += createDRow(allocatedWidth, "Borehole Spacing, m:", round(ghe.B_spacing, roundingAmount), stringFormat,
                     floatFormat)
    oS += createDRow(allocatedWidth, "Total Drilling, m:", round(bH.H * len(b), roundingAmount), stringFormat,
                     floatFormat)
    ...
    oS += createDRow(allocatedWidth, "Field Type:", ghe.fieldType, stringFormat, stringFormat, bTabs=indentedAmount)
    # oS += middleSpacingIndentedString.format("\t\tField Type:",ghe.fieldType,rO=rightAd,lO=leftAd)
    oS += createDRow(allocatedWidth, "Field Specifier:", ghe.fieldSpecifier, stringFormat, stringFormat,
                     bTabs=indentedAmount)
    # oS += middleSpacingIndentedString.format("\t\tField Specifier:",ghe.fieldSpecifier,rO=rightAd,lO=leftAd)
    oS += createDRow(allocatedWidth, "NBH:", len(b), stringFormat, intFormat, bTabs=indentedAmount)
    ...
    oS += createDRow(allocatedWidth, "Shank Spacing, m:", round(bhe.pipe.s, roundingAmount), stringFormat, floatFormat,
                     bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Pipe Outer Radius, m:", round(bhe.pipe.r_out, roundingAmount), stringFormat,
                     floatFormat,
                     bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Pipe Inner Radius, m:", round(bhe.pipe.r_in, roundingAmount), stringFormat,
                     floatFormat,
                     bTabs=indentedAmount)
    ...
    oS += createDRow(allocatedWidth, "Shank Spacing, m:", round(bhe.pipe.s, roundingAmount), stringFormat, floatFormat,
                     bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Grout Thermal Conductivity, W/(m*K):", round(bhe.grout.k, roundingAmount),
    ...
    oS += createDRow(allocatedWidth, "Reynold's Number:", round(BHE.compute_Reynolds(bhe.m_flow_borehole,
                                                                                     bhe.pipe.r_in, bhe.pipe.eps,
                                                                                     bhe.fluid), roundingAmount)
                     , stringFormat, floatFormat, bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Effective Borehole Resistance, W/(m*K):",
    ...
    oS += "Soil Properties: " + "\n"
    oS += createDRow(allocatedWidth, "Thermal Conductivity, W/(m*K):", round(bhe.soil.k, roundingAmount), stringFormat,
                     floatFormat, bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Volumetric Heat Capacity, kJ/(K*m^3):",
                     round(bhe.soil.rhoCp / 1000, roundingAmount), stringFormat, floatFormat, bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Undisturbed Ground Temperature, C:", round(bhe.soil.ugt, roundingAmount),
                     stringFormat, floatFormat, bTabs=indentedAmount)

    oS += "Fluid Properties" + "\n"
    oS += createDRow(allocatedWidth, "Volumetric Heat Capacity, kJ/(K*m^3):",
                     round(bhe.fluid.rhoCp / 1000, roundingAmount), stringFormat, floatFormat, bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Thermal Conductivity, W/(m*K):", round(bhe.fluid.k, roundingAmount), stringFormat,
                     floatFormat, bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Fluid Mix:", bhe.fluid.fluid_mix.split("::")[-1], stringFormat, stringFormat,
                     bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Density, kg/m^3:", round(bhe.fluid.rho, roundingAmount), stringFormat,
                     floatFormat, bTabs=indentedAmount)
    oS += createDRow(allocatedWidth, "Mass Flow Rate Per Borehole, kg/s:", round(bhe.m_flow_borehole, roundingAmount),
                     stringFormat, floatFormat, bTabs=indentedAmount)
    ...
    oS += createTable("GLHE Monthly Loads", monthHeader, monthlyLoadValues, allocatedWidth, monthTableFormats,
                      fillerSymbol="-", centering="^")

    oS += emptyLine

    oS += createTitle(allocatedWidth, "Simulation Parameters")
    oS += createDRow(allocatedWidth, "Start Month: ", ghe.sim_params.start_month, stringFormat, intFormat)
    oS += createDRow(allocatedWidth, "End Month: ", ghe.sim_params.end_month, stringFormat, intFormat)
    oS += createDRow(allocatedWidth, "Maximum Allowable HPEFT, C: ", ghe.sim_params.max_EFT_allowable, stringFormat,
                     floatFormat)
    oS += createDRow(allocatedWidth, "Minimum Allowable HPEFT, C: ", ghe.sim_params.min_EFT_allowable, stringFormat,
                     floatFormat)
    oS += createDRow(allocatedWidth, "Maximum Allowable Height, m: ", ghe.sim_params.max_Height, stringFormat,
                     floatFormat)
    oS += createDRow(allocatedWidth, "Minimum Allowable Height, m: ", ghe.sim_params.min_Height, stringFormat,
                     floatFormat)
    oS += createDRow(allocatedWidth, "Simulation Time, years: ", int(ghe.sim_params.end_month / 12), stringFormat,
                     intFormat)
        ...
        if currentMonth == lastMonth:
            ...
            if len(monthTbVals) > 0:
                ...
                if len(outArray) == 0:
                    ...
                    previousTemp = bhe.soil.ugt
                outArray.append(
                    [currentMonth, previousTemp + monthTbVals[-1], np.max(monthEFTVals), np.min(monthEFTVals)])
    ...
    oS += createDRow(allocatedWidth, "Max HPEFT, C:", round(maxEFT, roundingAmount), stringFormat, floatFormat)
    oS += createDRow(allocatedWidth, "Max HPEFT Time, Months:", round(maxEFTTime, roundingAmount), stringFormat,
                     floatFormat)
    oS += createDRow(allocatedWidth, "Min HPEFT, C:", round(minEFT, roundingAmount), stringFormat, floatFormat)
    oS += createDRow(allocatedWidth, "Min HPEFT Time, Months:", round(minEFTTime, roundingAmount), stringFormat,
                     floatFormat)
    ...
    with open(csvF1, "w", newline="") as csv1OF:
        ...
        cW.writerow(
            ["Time (hr)", "Time (month)", "Q (Rejection) (w) (before time)", "Q (Rejection) (W/m) (before time)",
             "Tb (C)", "GHE ExFT (C)"])
    ...
    csv3Array = []
    csv3Array.append(["Month", "Day", "Hour", "Time (Hours)", "Loading (W) (Extraction)"])
