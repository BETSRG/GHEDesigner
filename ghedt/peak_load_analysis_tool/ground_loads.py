...
def synthetic_load_function(
    ...
    peak_load_start_hour = math.floor(peak_day_noon - (float(peak_load_duration) / 2.0))
    hourly_loads[peak_load_start_hour : peak_load_start_hour + peak_load_duration] = [
        peak_load
    ] * peak_load_duration
...
def create_synthetic_doubling_load_profile(units="W", year=2019) -> tuple:
    ...
    days_in_month = [0] + [monthrange(year, i)[1] for i in range(1, 13)]
    ...
    hourly_extraction_loads: list = [0.0] * days_in_year * hours_in_day
    hourly_rejection_loads: list = [0.0] * days_in_year * hours_in_day
    ...
    for i in range(1, len(days_in_month)):
        ...
        peak_load_ext: float = 2.0 * float(i) * scale  # The monthly peak load
        peak_load_duration_ext: int = i  # The peak load duration

        # The peak load and peak load duration for rejection
        peak_load_rej: float = 2.0 * float(len(days_in_month) - i) * scale
...
class HybridLoad:
    ...
    def hourly_load_representation(self, year=2019):
        ...
        for i in range(1, len(days_in_month)):
            ...
            # duration of monthly peak htg load in hours
            peak_load_duration_ext: int = int(round(self.monthly_peak_hl_duration[i]))
            # day of the month on which peak htg load occurs (e.g. 1-31)
            peak_day_ext: int = self.monthly_peak_hl_day[i]

            peak_load_rej: float = self.monthly_peak_cl[i]
            peak_load_duration_rej: int = int(round(self.monthly_peak_cl_duration[i]))
    ...
    def visualize_fluid_simulation_results(
        self, month_num, extraction=True, rejection=True
    ):
        ...
        def draw_horizontal(two_day_fluid_temps_nm, peak_duration):
            ...
            points = [
                (peak_duration, two_day_fluid_nm_max),
