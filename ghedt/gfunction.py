    ...
    def visualize_g_functions(self):
        ...
        **fig, ax**
        ...
        for key in keys:
            ...
            if key == 8:
                ax.annotate(
                    str(round(float(self.B) / float(key), 4)), xy=(x_n - 0.4, y_n - 5)
                )
            else:
                ax.annotate(
                    str(round(float(self.B) / float(key), 4)), xy=(x_n - 0.4, y_n + 1)
                )
        ...
        fig.subplots_adjust(left=0.09, right=0.835, bottom=0.1, top=0.99)
