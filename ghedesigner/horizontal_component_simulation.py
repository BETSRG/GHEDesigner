...
def run_horizontal_simulation(config):
    ...
    try:
        ...
        # Force r_pipe to mathematically align with the beta provided in the config
        beta = config.get("beta", 0.344)
        # r_pipe = beta / (2 * np.pi * soil.k)

        table_single = lib_data["table_single"]
        table_parallel = lib_data["table_parallel"]
        horiz_axes = lib_data["axes"]

        # Snap to nearest axes to prevent floating point mismatch
        ...
        # We construct the path using the string provided in the config
