...
class TestHeatPump(TestCase):
    def test_fixed_cop_heating(self):
        ...
        load = lambda _: 1000  # 1 kW, positive indicating heating the building, taking energy from loop
        heat_pump.set_loads_from_lambda(load)
    ...
    def test_fixed_cop_cooling(self):
        heat_pump = HeatPump("load 1")
        load = lambda _: -1000  # 1 kW, negative indicating cooling the building, add energy to the loop
        heat_pump.set_loads_from_lambda(load)
    ...
    def test_load_lookup(self):
        ...
        self.assertNotEqual(outlet_temp, inlet_temp)