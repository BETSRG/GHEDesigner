...
class TestFindBiRectangleDesign(GHEBaseTest):
    ...
    def test_single_u_tube(self):
        ...
        self.assertAlmostEqual(132.44, u_tube_height, delta=0.01)
        nbh = ghe.results.borehole_location_data_rows  # includes a header row
        self.assertEqual(125 + 1, len(nbh))
    ...
    def test_double_u_tube(self):
        ...
        ghe.write_output_files(output_file_directory, "")
        u_tube_height = ghe.results.output_dict['ghe_system']['active_borehole_length']['value']
        self.assertAlmostEqual(132.68, u_tube_height, delta=0.01)
        nbh = ghe.results.borehole_location_data_rows  # includes a header row
        self.assertEqual(110 + 1, len(nbh))
