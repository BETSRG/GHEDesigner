...
class TestLoadAgg(GHEBaseTest):
    ...
    def test_all_timestep_types_agree(self):
        """Verify HOURLY, HYBRID, and HOURLYWITHLOADAGG all agree within 0.5°C for 1 year."""
        # Limit to 1 year so the HOURLY method completes in reasonable time
        num_months_1yr = 12

        ghe_hourly = self._make_ghe(num_months=num_months_1yr)
        ghe_hybrid = self._make_ghe(num_months=num_months_1yr)
        ...
        max_eft_hybrid, min_eft_hybrid = ghe_hybrid.simulate(method=TimestepType.HYBRID)
        ...
        self.assertAlmostEqual(max_eft_hybrid, max_eft_agg, delta=0.5)
        self.assertAlmostEqual(min_eft_hybrid, min_eft_agg, delta=0.5)